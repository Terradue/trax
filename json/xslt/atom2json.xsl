...
<xsl:template match="atom:*" mode="atomCommonAttributes">
	<xsl:apply-templates select="@xml:base | @xml:lang"/><xsl:value-of select="concat($new-line,$tab)"/>
</xsl:template>
...
<xsl:template match="atom:author | atom:category | atom:contributor | atom:link ">
		<xsl:for-each select="@* | atom:*"><xsl:apply-templates select="."/><xsl:if test="position() &lt; last()">,</xsl:if><xsl:value-of select="concat($new-line,$tab,$tab)"/><xsl:if test="name(../..)='entry'"><xsl:value-of select="$tab"/></xsl:if></xsl:for-each>
</xsl:template>
...
<xsl:template match="atom:entry">
		...
		<xsl:for-each select="atom:*[name()!='author' and name()!='category' and name()!='contributor' and name()!='link']"><xsl:apply-templates select="."/>,<xsl:if test="position() &lt; last()"><xsl:value-of select="concat($new-line,$tab,$tab)"/></xsl:if></xsl:for-each>
