...
<!-- rng2sch
	Modified schematron extractor (ExtractSchFromRNG.xsl) code by Pedro Goncalves.
	The changes allow the direct use of this xsl with sch extracted from a RNG 
	file with foreign elements and without the sch:pattern element
...
| $includes//rng:externalRef/@href)"/>
	...
	<xsl:template name="output">
		...
		<sch:schema>
			...
			<xsl:if test="count($schemas//sch:pattern)=0 and count($schemas//sch:rule)!=0">
			<sch:pattern><xsl:copy-of select="$schemas//sch:rule"/></sch:pattern>
			</xsl:if>
			<sch:diagnostics>
